...
export function MenuTab({
    ...
    const [categoryFilter, setCategoryFilter] = useState<'all' | 'cafes' | 'postres' | 'especiales'>('all');
    ...
    const handleCategoryChange = (cat: 'all' | 'cafes' | 'postres' | 'especiales') => {
    ...
    return (
        <div className="space-y-6 sm:space-y-8">
            {/* Formulario desacoplado (Cero re-renders en el catálogo al tipear) */}
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
                ...
                <div className="flex items-center gap-1.5 bg-[#14110E] p-1 rounded-2xl border border-[#2D2620] w-full sm:w-auto overflow-x-auto">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold transition-all cursor-pointer shrink-0 ${categoryFilter === 'all' ? 'bg-[#231F1B] text-white' : 'text-[#A39B92] hover:text-white'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold transition-all cursor-pointer shrink-0 ${categoryFilter === 'cafes' ? 'bg-[#231F1B] text-white' : 'text-[#A39B92] hover:text-white'
                            }`}
                    >
                        Cafés
                    </button>
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold transition-all cursor-pointer shrink-0 ${categoryFilter === 'postres' ? 'bg-[#231F1B] text-white' : 'text-[#A39B92] hover:text-white'
                            }`}
                    >
                        Postres
                    </button>
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold transition-all cursor-pointer shrink-0 ${categoryFilter === 'especiales' ? 'bg-[#231F1B] text-white' : 'text-[#A39B92] hover:text-white'
                            }`}
                    >
                        Especiales
                    </button>
            ...
            {loading ? (
                ...
                <div className="p-10 text-center rounded-3xl bg-[#14110E] border border-[#2D2620] space-y-2">
                    ...
                    <p className="text-xs text-[#A39B92]">Intenta cambiar el término de búsqueda o la categoría.</p>
                </div>
            ) : (
                <>
