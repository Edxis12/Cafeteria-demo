...
function AlertToast({ alertText }: { alertText: string }) {
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="fixed top-4 left-3 right-3 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-50 bg-emerald-500 text-black px-4 sm:px-6 py-2.5 rounded-full font-bold text-xs shadow-2xl flex items-center justify-center gap-2 pointer-events-none"
        >
...
export function AdminHeader({
  ...
  return (
    <>
      {newArrivalAlert && (
        <AlertToast key={newArrivalAlert} alertText={newArrivalAlert} />
      )}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-[#A39B92] hidden md:block">
                Control de Reservas & Catálogo
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5">
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all cursor-pointer min-h-[36px] ${
                notificationsEnabled
            ...
            >
              <RefreshCw
                size={13}
                className={loading ? "animate-spin text-[#D57E7E]" : ""}
              />
