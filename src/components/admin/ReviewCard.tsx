...
export const ReviewCard = React.memo(function ReviewCard({
  ...
  return (
    <div
      className={`p-5 sm:p-6 rounded-3xl bg-[#14110E] border space-y-4 shadow-xl transition-all flex flex-col justify-between ${
        rev.is_approved
    ...
    >
      <div className="space-y-3">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            className={`text-[9px] px-2.5 py-1 rounded-full uppercase font-bold tracking-wider flex items-center gap-1 shrink-0 ${
              rev.is_approved
        ...
        {/* Comentario con comillas escapadas */}
        <p className="text-xs sm:text-sm text-[#C5BCB3] italic leading-relaxed">
          &ldquo;{unescapeHtml(rev.comment)}&rdquo;
        </p>
      </div>
