...
export function EditMenuModal({ item, onClose, onUpdate, isUpdating }: EditMenuModalProps) {
    ...
    const [featuredOrder, setFeaturedOrder] = useState<number>(item.featured_order || 1);
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 z-50 overflow-y-auto">
            <div className="bg-[#14110E] border border-[#2D2620] rounded-3xl p-5 sm:p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto space-y-4 shadow-2xl my-auto">
                <div className="flex justify-between items-center border-b border-[#2D2620] pb-3">
                    <h3 className="font-serif font-bold text-base sm:text-lg text-white">Editar Producto</h3>
                    <button onClick={onClose} className="text-[#A39B92] hover:text-white cursor-pointer p-1">
                        <X size={20} />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-3.5 sm:space-y-4">
                    <div>
                        <label className="block text-xs font-semibold text-[#A39B92] mb-1">Nombre</label>
                        <input
                            type="text"
                            value={title}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        <div>
                            <label className="block text-xs font-semibold text-[#A39B92] mb-1">Precio (MXN)</label>
                            <input
                                type="number"
                                step="0.01"
                                min="0"
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-[#A39B92] mb-1">Categoría</label>
                            <select
                                value={category}
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-[#A39B92] mb-1">Descripción</label>
                        <textarea
                            value={desc}
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-[#A39B92] mb-1">Imagen del Producto</label>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                            <label className="flex items-center gap-2 cursor-pointer select-none">
                                ...
                                <span className="text-xs font-bold text-white flex items-center gap-1">
                                    <Flame size={13} className="text-orange-400" /> Marcar como Más Vendido
                                </span>
                            </label>
