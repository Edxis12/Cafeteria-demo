...
import { Star, X, CheckCircle2, MessageSquarePlus, Loader2 } from 'lucide-react';
...
export function ReviewModal({ isOpen, onClose }: ReviewModalProps) {
    ...
    // Cancelar peticiones pendientes si el modal se desmonta
    useEffect(() => {
        return () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Configuración de AbortController con timeout de seguridad (10s)
        ...
        try {
            ...
            if (res.ok && result.success) {
                ...
                const firstError =
                    result.details?.[0]?.message || result.error || 'Hubo un error al enviar tu reseña.';
                setErrorMessage(firstError);
            }
        } catch (error: any) {
            clearTimeout(timeoutId);
            if (error.name === 'AbortError') {
                setErrorMessage('La solicitud tardó demasiado tiempo. Verifica tu conexión.');
            } else {
        ...
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
                    {/* Overlay interactivo para cerrar */}
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                    ...
                    >
                        ...
                        {isSuccess ? (
                            <div className="text-center py-6 space-y-4">
                                ...
                                <h3 className="font-serif font-bold text-xl text-white">¡Gracias por tu opinión!</h3>
                                <p className="text-xs text-[#A39B92] leading-relaxed">
                                    Hemos recibido tu reseña con éxito. Para mantener la calidad de la comunidad, se publicará en breve tras una breve verificación.
                                </p>
                                <button
                                    onClick={handleClose}
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-xs font-semibold text-[#C5BCB3]">Calificación</label>
                                ...
                                {/* Nombre */}
                                <div className="space-y-1">
                                    <label className="block text-xs font-semibold text-[#C5BCB3]">Tu Nombre</label>
                                    <input
                                        type="text"
                                ...
                                {/* Comentario */}
                                <div className="space-y-1">
                                    <label className="block text-xs font-semibold text-[#C5BCB3]">Tu Comentario</label>
                                    <textarea
